...
def monitor_pr(pr_number: int, check_interval: int = 10):
    ...
    while True:
        ...
        print(f"[{timestamp}] 📡 チェック #{check_count} - PR情報を取得中...", flush=True)
        ...
        print(f"[{timestamp}] 📋 PR情報: '{pr_title}' (状態: {pr_state}, マージ可能: {mergeable})", flush=True)
        ...
        if len(comments) > last_comment_count:
            ...
            print(f"[{timestamp}] 📈 コメント数: {last_comment_count} → {len(comments)}", flush=True)
...
def main():
    ...
    try:
        ...
        print("❌ エラー: GitHub CLI (gh) がインストールされていないか、認証されていません", flush=True)
