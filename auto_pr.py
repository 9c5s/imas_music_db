...
class AutoPR:
    ...
    def create_pr(self, title: str, body: str) -> Optional[int]:
        """PRを作成して番号を返す"""
        print("🚀 プルリクエストを作成中...")
    ...
    def _run_monitor(self, pr_number: int):
        ...
        try:
            ...
            subprocess.run([
                "uv", "run", "python", "monitor_pr.py", str(pr_number)
            ], check=True)
    ...
    def generate_default_pr_info(self) -> tuple:
        ...
        if branch.startswith("feature/"):
            ...
            title = "バグ修正: " + branch.replace("fix/", "").replace("-", " ").replace("_", " ")
        else:
            title = f"{branch} の変更"
        ...
        if success and commits:
            ...
            body_parts.extend([f"- {commit}" for commit in commit_list if commit.strip()])
        else:
    ...
    def run(self, title: Optional[str] = None, body: Optional[str] = None):
        """メイン処理"""
        print("🤖 PR自動作成・監視ツール")
        print("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PR作成と監視を自動化するスクリプト"
    )
    parser.add_argument(
        "--title", "-t",
        help="PRタイトル（指定しない場合はブランチ名から自動生成）"
    )
    parser.add_argument(
        "--body", "-b", 
        help="PR説明（指定しない場合は最近のコミットから自動生成）"
    )
    parser.add_argument(
        "--no-monitor", "-n",
        action="store_true",
